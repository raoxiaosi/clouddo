...
import com.bootdo.clouddoadmin.pojo.domain.RoleMenuDO;
...
public class RoleServiceImpl implements RoleService {
    ...
    @Override
    public List<UserRoleVO> list(Long userId) {
        List<Long> rolesIds = userRoleMapper.listRoleId(userId);
        List<RoleDO> roles = roleMapper.list(new HashMap<>(16));
        List<UserRoleVO> roleVOList = new ArrayList<>();
        for (RoleDO roleDO : roles) {
            UserRoleVO userRoleVO = new UserRoleVO();
            userRoleVO.setId(roleDO.getRoleId());
            userRoleVO.setName(roleDO.getRoleName());

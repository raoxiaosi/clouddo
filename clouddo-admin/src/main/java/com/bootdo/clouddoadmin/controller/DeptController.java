...
//	}
	...
	@PostMapping("/save")
	@RequiresPermissions("system:sysDept:add")
	public ApiResult save(DeptDO sysDept) {
	...
	@RequiresPermissions("system:sysDept:edit")
	public ApiResult update(DeptDO sysDept) {
	...
	public ApiResult remove(Long deptId) {
		...
		if(sysDeptService.count(map)>0) {
			return ApiResult.error(1, "包含下级部门,不允许修改");
		}
		if(sysDeptService.checkDeptHasUser(deptId)) {
			...
			return ApiResult.error(1, "部门包含用户,不允许修改");
		}
		return ApiResult.error();
	}
	...
	public ApiResult remove(@RequestParam("ids[]") Long[] deptIds) {
